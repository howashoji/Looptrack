...
逆に、確認する側に倒れるものが 3 つあります。誤発火ではありますが、秘密を通すよりは安全な側です。
`public` と `ca` が同居する公開の名前（`public-ca-bundle.pem`）と、`key` をたまたま含む語（`monkey.pem`）と、
**名前が見えているグロブ**（`cat /tmp/x/.env*` は `.env.example` のような雛形にも当たるが、末尾のグロブを
落とした残り `.env` が秘密の名前そのものなので確認します）です。
...
- **名前を伏せるグロブ**: `cat /tmp/x/*`・`cat /tmp/x/.en?`。文字どおりの部分が秘密を名乗っていません。
  **名前が見えているグロブ（`cat /tmp/x/.env*`・`cat /tmp/x/id_rsa*`）は捕まえます**（末尾のグロブ
  `*`・`?`・`[…]` を落とした残りが、名前だけで秘密と分かるもの（`.env`・`id_rsa` 系など）に丸ごと当たれば確認します。
  中身を出す・別の場所へ写す読むコマンドに限るので、`ls /tmp/x/.env*` のような一覧だけのコマンドでは鳴りません）。
